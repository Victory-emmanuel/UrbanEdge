              ...
              {property.floorPlans ? (
                <div className="space-y-6">
                  {property.floorPlans.map((plan, index) => (
                    <div key={index} className="bg-white dark:bg-brown-dark rounded-lg shadow-md overflow-hidden">
                      <div className="p-4 border-b border-beige-medium dark:border-brown">
                        ...
                        <p className="text-sm text-brown dark:text-beige-medium">
                          {plan.size} sqft • {plan.bedrooms} Bed • {plan.bathrooms} Bath
                        </p>
                      </div>
                      <div className="p-4">
                        <img
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {property.neighborhood.nearbyPlaces.map((place, index) => (
                        <div key={index} className="bg-white dark:bg-brown-dark p-3 rounded-lg shadow-sm">
              ...
              {property.reviews && property.reviews.length > 0 ? (
                <div className="space-y-6">
                  {property.reviews.map((review, index) => (
                    <div key={index} className="bg-white dark:bg-brown-dark p-4 rounded-lg shadow-md">
                      ...
                      <div className="flex mb-2">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
                            className={`h-5 w-5 ${
                              i < review.rating ? "text-taupe" : "text-beige-medium dark:text-brown"
                            }`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
